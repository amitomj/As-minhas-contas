  ...
  return (
    <div className="h-full flex flex-col pb-32 overflow-y-auto no-scrollbar relative z-10">
      <header className="flex items-center justify-between px-6 pt-12 pb-4 shrink-0 z-50">
        <div className="flex items-center gap-3 p-2" onClick={() => setView('settings')}>
        ...
        <button onClick={onLogout} className="h-12 w-12 flex items-center justify-center rounded-xl bg-white/5 border border-white/5 text-red-400">
      ...
      <main className="flex-1 px-6 space-y-10 relative z-20">
        ...
        <section className="grid grid-cols-2 gap-4">
          <button 
            ...
            className="flex flex-col items-center justify-center py-10 rounded-[2.5rem] bg-gradient-to-br from-secondary to-primary text-white shadow-2xl"
          >
          ...
          <button 
            ...
            className="flex flex-col items-center justify-center py-10 rounded-[2.5rem] bg-surface-dark border border-white/5 text-white shadow-lg"
          >
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setView('transactions')} className="text-[10px] text-primary font-black uppercase tracking-widest p-3 bg-primary/5 rounded-xl">
              Ver Tudo
            </button>
          </div>
              ...
              <div key={exp.id} className="relative group">
                ...
                {actionMenuId === exp.id && (
                  <div className="absolute right-4 top-16 bg-bg-dark border border-white/10 rounded-2xl shadow-2xl z-50 p-2 min-w-[140px] animate-in fade-in zoom-in-95">
                    <button onClick={(e) => { e.stopPropagation(); onEdit(exp); setActionMenuId(null); }} className="w-full flex items-center gap-3 px-4 py-3 text-[11px] font-bold text-blue-400">
                      <span className="material-symbols-outlined text-sm">edit</span> Editar
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); if(confirm("Apagar?")) onDelete(exp.id); setActionMenuId(null); }} className="w-full flex items-center gap-3 px-4 py-3 text-[11px] font-bold text-red-400">
