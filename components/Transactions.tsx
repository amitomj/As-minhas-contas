...
// Utility for string normalization and fuzzy matching
 ...
 * Checks if query is a normalized substring or has high character overlap.
 */
const fuzzyMatch = (text: string, query: string) => {
  ...
  // 2. Simple letter-by-letter subsequence check (for minor typos or abbreviations)
  let qIdx = 0;
  for (let tIdx = 0; tIdx < nText.length && qIdx < nQuery.length; tIdx++) {
    if (nText[tIdx] === nQuery[qIdx]) qIdx++;
  }
  
  // If we matched at least 80% of query characters in order, consider it a match
...
const Transactions: React.FC<Props> = ({ expenses, members, projects = [], onBack, onEdit, onDelete }) => {
  ...
  const filteredExpenses = useMemo(() => {
    return expenses
      .filter(e => {
        // Use fuzzyMatch for search
        const matchesSearch = !search.trim() || 
                              fuzzyMatch(e.source, search) || 
                              fuzzyMatch(e.notes || '', search);
        
        const matchesMember = filterMember === 'all' || 
                              (filterMember === 'none' && e.memberIds.length === 0) ||
                              (filterMember !== 'all' && filterMember !== 'none' && e.memberIds.includes(filterMember));
        
        const matchesSource = filterSource === 'all' || e.source === filterSource;

        const matchesProject = filterProject === 'all' || 
                               (filterProject === 'none' && !e.projectId) ||
                               (filterProject !== 'all' && filterProject !== 'none' && e.projectId === filterProject);
  ...
  return (
    <div className="h-full bg-[#050c09] flex flex-col pb-24 overflow-hidden">
      <header className="px-6 pt-12 pb-4 space-y-4 shrink-0 bg-[#050c09]/80 backdrop-blur-md z-10 border-b border-white/5">
        ...
        {showFilters && (
          <div className="p-6 bg-[#11291f] rounded-[2.5rem] border border-white/10 space-y-4 animate-in fade-in slide-in-from-top-4 shadow-2xl overflow-y-auto max-h-[50vh] no-scrollbar">
